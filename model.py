...
class Encoder(torch.nn.Module):
    ...
    def forward(self, x, edge_index, not_prop=0):
        if args.model == 'GNAE':
            ...
            x = self.linear2(x)  # 新增：将 hidden_channels 转换为 out_channels
            return x

        if args.model == 'VGNAE':
            x_ = self.linear1(x)
            x_ = self.propagate(x_, edge_index)
            x_ = self.linear2(x_)  # 调整均值路径的输出维度

            x = self.linear1(x)
            x = F.normalize(x, p=2, dim=1) * args.scaling_factor
            x = self.propagate(x, edge_index)
            x = self.linear2(x)  # 调整方差路径的输出维度
            return x
...
class DegreeDecoder(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels=1):
        ...
        # 参数调整以适配KANLayer的要求
        # in_channels, hidden_channels 用于定义KANLayer中的维度配置
        # out_channels 用于定义输出的特征维度
        self.kan_layer = KANLayer(
            in_dim=in_channels,
            out_dim=out_channels,
            num=hidden_channels,  # 使用hidden_channels作为网格数，需要调整以适应模型设计
            k=3,  # B样条的阶数，可以根据需要进行调整
            noise_scale=0.1,
            scale_base=1.0,
            scale_sp=1.0,
            base_fun=torch.nn.SiLU(),  # 选择适合的基函数
            grid_eps=0.02,
            grid_range=[-1, 1],  # 网格范围
            sp_trainable=True,
            sb_trainable=True
        ...
        self.dropout = nn.Dropout(0.5)  # 保持dropout层
        self.activation = nn.ELU()  # 使用ELU作为激活函数
...
class GMAE(nn.Module):
    ...
    @torch.no_grad()
    def test(self, z, pos_edge_index, neg_edge_index, specific_edges=None):
        # 预测正类和负类
        pos_pred = self.batch_predict(z, pos_edge_index)
        neg_pred = self.batch_predict(z, neg_edge_index)

        if specific_edges is not None:
            specific_pred = self.batch_predict(z, specific_edges)
            # 假设所有特定边都是正边，根据实际情况调整
            specific_true = torch.ones(specific_pred.size(0))
            specific_auc = roc_auc_score(specific_true.cpu().numpy(), specific_pred.cpu().numpy())
            print("Specific edges AUC:", specific_auc)
